...
var _warp = _interopRequireDefault(require("./modules/warp"));
...
},{"./modules/warp":35}],35:[function(require,module,exports){
"use strict";
...
var _default = {
  init: function init() {
    ...
    if (is_safari) {
      document.querySelector('footer').style.paddingTop = '0.5em';
    }
    ...
    var radius = 475; // init transisiton

    var t = d3.transition().duration(1000).ease(d3.easePolyIn); // effect switch

    var effectIsActive = false; // init polygon lists

    var newPolygonList = [];
    var oldPolygonList = []; // populate polygon lists

    d3.selectAll('.Header-reference polygon').each(function (d, i, g) {
      // create new point list for each polygon
      var points = g[i].points;
      var newPointList = [];
      var oldPointList = []; // go through all points, to find it's new position

      for (var j = 0; j < points.length - 1 || j < points.numberOfItems - 1; j++) {
        // get current point, distance and angle
        var point = points[j] || g[i].points.getItem(j);
        var dist = distBetweenPoints(center, point);
        var angle = angleBetweenPoints(center, point); // max point and distance

        var maxPoint = rectPoint(rect, angle);
        var maxDist = distBetweenPoints(center, maxPoint); // min bound point and distance

        var minBoundPoint = circPoint(radius, angle);
        var minBoundDist = distBetweenPoints(center, minBoundPoint); // max bound point and distance

        var maxBoundPoint = rectPoint(maxRect, angle);
        maxBoundPoint.x -= (maxRect.width - rect.width) / 2;
        maxBoundPoint.y -= (maxRect.height - rect.height) / 2;
        var maxBoundDist = distBetweenPoints(center, maxBoundPoint); // scale

        var scale = d3.scaleLinear().domain([0, maxBoundDist]).range([minBoundDist, maxBoundDist]); // new dist

        var newDist = scale(dist, 0, maxDist, minBoundDist, maxBoundDist); // get new distance using scale and bounds

        var newPoint = {
          x: center.x + Math.cos(angle) * newDist,
          y: center.y + Math.sin(angle) * newDist
        }; // add new point to the list

        newPointList.push(newPoint);
        oldPointList.push(point);
      } // add new polygon to the list


      newPolygonList.push(newPointList);
      oldPolygonList.push(oldPointList);
    }); // build curved shapes

    var lineGenerator = d3.line().curve(d3.curveCatmullRomClosed.alpha(0.5));
    console.log(oldPolygonList); // create paths in logo based on the reference

    d3.select('.Header-logo').selectAll('path').data(oldPolygonList).enter().append('path').attr('d', function (d, i) {
      var points = d.map(function (poly) {
        return [poly.x, poly.y];
      });
      var pathData = lineGenerator(points);
      return pathData;
    }).attr('data-link', function (d, i) {
      if (i == 15) {
        return 'https://www.google.com/maps/place/Green+Hall,+1156+Chapel+St,+New+Haven,+CT+06511/@41.308332,-72.9351456,17z/data=!3m1!4b1!4m5!3m4!1s0x89e7d9b2bed3cfd1:0xc8ad7bcf604df0f2!8m2!3d41.308332!4d-72.9329516';
      }
    });
    d3.selectAll('.Header-logo path[data-link="https://www.google.com/maps/place/Green+Hall,+1156+Chapel+St,+New+Haven,+CT+06511/@41.308332,-72.9351456,17z/data=!3m1!4b1!4m5!3m4!1s0x89e7d9b2bed3cfd1:0xc8ad7bcf604df0f2!8m2!3d41.308332!4d-72.9329516"]').each(function () {
      var _this = this;

      d3.select(this.parentNode).insert('a', function () {
        return _this;
      }).attr('xlink:href', 'https://www.google.com/maps/place/Green+Hall,+1156+Chapel+St,+New+Haven,+CT+06511/@41.308332,-72.9351456,17z/data=!3m1!4b1!4m5!3m4!1s0x89e7d9b2bed3cfd1:0xc8ad7bcf604df0f2!8m2!3d41.308332!4d-72.9329516').attr('target', 'blank').append(function () {
        return _this;
      });
    }); // create circles in logo based on the reference

    d3.select('.Header-logo').selectAll('g').data(oldPolygonList).enter().append('g').selectAll('circle').data(function (d) {
      return d;
    ...
    setTimeout(function () {
      if (md.mobile()) {
        ...
        document.addEventListener('scroll', function (e) {
          if (window.scrollY > scrollTop) {
            if (!effectIsActive) {
              ...
              transformPaths(newPolygonList);
            }
          } else {
            if (effectIsActive) {
              effectIsActive = !effectIsActive;
              transformPaths(oldPolygonList);
            }
          }
        ...
        document.querySelector('.Content').addEventListener('mouseenter', function (e) {
          if (!effectIsActive) {
            effectIsActive = !effectIsActive;
            transformPaths(newPolygonList);
          }
        ...
        document.querySelector('.Content').addEventListener('mouseleave', function (e) {
          if (effectIsActive) {
            effectIsActive = !effectIsActive;
            transformPaths(oldPolygonList);
          }
    ...
    function transformPaths(data) {
      // edit paths
      d3.select('.Header-logo').selectAll('path').data(data).interrupt().transition(t).attr('d', function (d) {
        // create and return the line data from points array 
        var points = d.map(function (poly) {
          return [poly.x, poly.y];
        });
        var pathData = lineGenerator(points);
      ...
      d3.select('.Header-logo').selectAll('g').data(data).selectAll('circle').data(function (d) {
        return d;
      }).interrupt().transition(t).attr("cx", function (d) {
        return d.x;
      }).attr("cy", function (d) {
        return d.y;
      }).attr("r", function (d) {
    ...
    function rectPoint(rect, angle) {
      ...
      var vy = Math.sin(angle); // test for shortest distance to bounds
      ...
      if (vy) {
        t.push((y1 - py) / vy);
        t.push((y2 - py) / vy);
      } // find smallest greater than 0


      t = t.filter(function (e) {
        return e > 0;
      });
      t = Math.min.apply(Math, _toConsumableArray(t)); // return point
    ...
    function circPoint(radius, angle) {
      return {
